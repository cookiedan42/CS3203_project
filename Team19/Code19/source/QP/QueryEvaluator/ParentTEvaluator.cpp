...
ParentTEvaluator::ParentTEvaluator(Argument& arg1, Argument& arg2, GetFacade& facade) :
...
*/
bool ParentTEvaluator::evaluateClauseWithNoSyn()
{
...
*/
RelationshipResults ParentTEvaluator::getRelationPairs()
{
	...
	if (type == ArgsType::STMTNO_STMTNO)
	{
		...
		auto request = std::make_shared<GetParentRequest>(arg1Value, arg2Value, Mode::FILTER_ARG1_ARG2, STAR);
	...
	if (type == ArgsType::STMTNO_WILDCARD || type == ArgsType::STMTNO_SYN)
	{
		...
		ArgString arg1Value = std::get<StmtNo>(arg1.argumentVariant).value;
		auto request = std::make_shared<GetParentRequest>(arg1Value, Mode::FILTER_ARG_1, STAR);
		result = facade.get(request);
	}
	if (type == ArgsType::SYN_STMTNO || type == ArgsType::WILDCARD_STMTNO)
	{
		...
		ArgString arg2Value = std::get<StmtNo>(arg2.argumentVariant).value;
		auto request = std::make_shared<GetParentRequest>(arg2Value, Mode::FILTER_ARG_2, STAR);
		result = facade.get(request);
	}
