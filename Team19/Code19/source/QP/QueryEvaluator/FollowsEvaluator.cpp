...
FollowsEvaluator::FollowsEvaluator(Argument& arg1, Argument& arg2, GetFacade& facade) :
...
*/
bool FollowsEvaluator::evaluateClauseWithNoSyn()
{
...
*/
RelationshipResults FollowsEvaluator::getRelationPairs()
{
	...
	if (type == ArgsType::STMTNO_STMTNO) 
	{
		...
		auto request = std::make_shared<GetFollowsRequest>(arg1Value, arg2Value, Mode::FILTER_ARG1_ARG2);
	...
	if (type == ArgsType::STMTNO_WILDCARD || type == ArgsType::STMTNO_SYN) 
	{
		...
		ArgString arg1Value = std::get<StmtNo>(arg1.argumentVariant).value;
		auto request = std::make_shared<GetFollowsRequest>(arg1Value, Mode::FILTER_ARG_1);
		result = facade.get(request);
	}
	if (type == ArgsType::SYN_STMTNO || type == ArgsType::WILDCARD_STMTNO) 
	{
		...
		ArgString arg2Value = std::get<StmtNo>(arg2.argumentVariant).value;
		auto request = std::make_shared<GetFollowsRequest>(arg2Value, Mode::FILTER_ARG_2);
		result = facade.get(request);
	}
