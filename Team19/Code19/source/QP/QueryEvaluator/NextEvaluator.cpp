#include "NextEvaluator.h"

NextEvaluator::NextEvaluator(Argument& arg1, Argument& arg2, GetFacade& facade) :
	RelationshipEvaluator(arg1, arg2, facade) {}

/**
* Evaluate clauses that have no synonyms.
*
* @param rel Next design abstraction to be evaluated
* @param facade Storage to be accessed
* @return boolean result if values exist for the design abstraction
*/
bool NextEvaluator::evaluateClauseWithNoSyn()
{
	RelationshipResults result = getRelationPairs();

	return !result.empty();

}

/**
* Get all relation pairs that satisfy the Next relationship.
*
* @param rel Next relationship to be evaluated
* @param facade Storage to be accessed
* @return pairs of entities that satisfy Next relationship
*/
RelationshipResults NextEvaluator::getRelationPairs()
{
	ArgsType type = QueryUtil::getArgumentsTypesFromMap(arg1, arg2);
	RelationshipResults result;

	if (type == ArgsType::SYN_SYN) {
		if (QueryUtil::isSameSynonym(arg1, arg2)) {
			return result;
		}
	}

	if (type == ArgsType::SYN_SYN || type == ArgsType::SYN_WILDCARD || 
		type == ArgsType::WILDCARD_SYN || type == ArgsType::WILDCARD_WILDCARD)
	{
		// eg. statement p, q; Next(p, q), Next(p, _), Next(_, q), Next(_, _)
		auto request = std::make_shared<GetNextRequest>(Mode::ALL);
		result = facade.get(request);
	}
	if (type == ArgsType::SYN_STMTNO || type == ArgsType::WILDCARD_STMTNO)
	{
		// eg. Next(p, 1), Next(_, 1)
		ArgString arg2Value = std::get<StmtNo>(arg2.argumentVariant).value;
		auto request = std::make_shared<GetNextRequest>(arg2Value, Mode::FILTER_ARG_2);
		result = facade.get(request);
	}
	if (type == ArgsType::STMTNO_SYN || type == ArgsType::STMTNO_WILDCARD)
	{
		// eg. Next(1, q), Next(1, _)
		ArgString arg1Value = std::get<StmtNo>(arg1.argumentVariant).value;
		auto request = std::make_shared<GetNextRequest>(arg1Value, Mode::FILTER_ARG_1);
		result = facade.get(request);
	}
	if (type == ArgsType::STMTNO_STMTNO)
	{
		// eg. Next(1, 2)
		ArgString arg1Value = std::get<StmtNo>(arg1.argumentVariant).value;
		ArgString arg2Value = std::get<StmtNo>(arg2.argumentVariant).value;
		auto request = std::make_shared<GetNextRequest>(arg1Value, arg2Value, Mode::FILTER_ARG1_ARG2);
		result = facade.get(request);
	}

	return result;
}
